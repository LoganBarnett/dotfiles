# I added speedtest-cli as an overlay because 1: it's kind of useful and 2: it's fairly
...
# consuming nix file it is simply presented as "pkgs.speedtest-cli". I might like to
# keep this around as an example.
self: super: {
  speedtest-cli = super.python37.pkgs.buildPythonApplication rec {
    pname = "speedtest-cli";
    version = "2.1.3";
    format = "other";

    pythonPath = [ super.python37.pkgs.setuptools ];
    nativeBuildInputs = [
      super.python37.pkgs.wrapPython
      super.makeWrapper
    ];

    src = super.python37.pkgs.fetchPypi {
      inherit pname version;
      sha256 = "1w4h7m0isbvfy4zx6m5j4594p5y4pjbpzsr0h4yzmdgd7hip69sy";
    };
    buildPhase = ''
      ${super.python37.interpreter} setup.py build
    '';

    installPhase = ''
      ${super.python37.interpreter} setup.py install --prefix="$out"
      for i in "$out/bin"/*; do
      head -n 1 "$i" | grep -E '[/ ]python( |$)' && {
        wrapProgram "$i" --prefix PYTHONPATH : "$PYTHONPATH:$out/${super.python37.sitePackages}"
      } || true
      done
    '';
