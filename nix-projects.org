...
This is all the Nix related projects I have.  Much of this is dated and I think
much of it I've overcome.  But I need a moment to groom it all.
...
[[https://github.com/NixOS/nixpkgs/issues/260754][nixpkgs#260754]] led me to using =raspberry-pi-nix= for building Raspberry Pi
...
** secrets
*** Secret generation (LLM guidance)

This repository uses =agenix-rekey= to generate and manage secrets.  Several
secret types — TLS leaf certificates, WireGuard keys, hashed passwords, and
others — are produced by generator scripts that run via:

#+begin_src sh :exports code
agenix rekey generate --rekey -a
#+end_src

This repository uses a custom fork of =agenix-rekey= that combines generation
and rekeying into a single subcommand.  Generating a TLS leaf certificate, for
example, requires decrypting the internal CA key, which is protected by an
interactive passphrase prompt.  *This command cannot be run non-interactively.*

*LLMs assisting with this repository must halt and prompt the user* whenever a
task requires generating or rekeying secrets.  Do not attempt to create
placeholder =.age= files, dummy certificates, or fake rekeyed entries as a
workaround.  Instead, stop and instruct the user to run the command above
themselves, then continue once the generated files have been committed.

The expected post-generation workflow for a new FQDN (e.g. adding a new
service to a host) is:

1. Author the NixOS module and config normally.
2. Halt and ask the user to run =agenix rekey generate --rekey -a= (generates
   =.crt=, =.key.age=, and the per-host rekeyed file in
   =secrets/rekeyed/<host>/=, and =git add=s them automatically).
3. The user commits the generated files.
4. Resume with the build/deploy once the secrets are in git.
