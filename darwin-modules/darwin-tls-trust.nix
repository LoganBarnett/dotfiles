  ...
  config = {
    system.activationScripts.postActivation.text =
      let
        ...
        santizedCertName =
          cert-path:
          if builtins.isPath cert-path then
            builtins.baseNameOf cert-path
          else
            (
              if builtins.hasContext cert-path then
                builtins.unsafeDiscardStringContext cert-path
              else
                builtins.baseNameOf cert-path
            );
        profile =
          pkgs.callPackage ../packages/darwin-tls-certificate-configuration-profile.nix
            {
