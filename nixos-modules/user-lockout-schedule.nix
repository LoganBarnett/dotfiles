  ...
  mkOnCalendars =
    ...
    concatLists (
      map (
        d:
        let
          times = lib.attrByPath [ d.key ] [ ] timesByDay;
        in
        map (t: "${d.name} ${t}") times
      ) weekdays
    );

  userExists = u: lib.hasAttr u config.users.users;

  logoutScript = ../scripts/user-logout-and-lockout;
  enableScript = ../scripts/user-login-enable;
    ...
    users = mkOption {
      type = types.attrsOf (
        types.submodule (
          ...
          {
            options = {
              enable = (mkEnableOption "This user's lockout schedule.") // {
                default = true;
              };
              schedule = mkOption {
  ...
  config =
    ...
    (
      let
        ...
        onCalsLogout =
          uCfg: mkOnCalendars (lib.mapAttrs (_: v: v.logoutAt) uCfg.schedule);
        onCalsEnable =
          uCfg: mkOnCalendars (lib.mapAttrs (_: v: v.enableAt) uCfg.schedule);
