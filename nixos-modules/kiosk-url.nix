...
in
{
  options.services.kiosk = {
    enable = lib.mkEnableOption "kiosk display";
    url = lib.mkOption {
      type = lib.types.str;
    ...
    browser = lib.mkOption {
      ...
      default = "chromium";
      description = "Browser to use for the kiosk display.";
    };
  };
        ...
        ExecStart =
          let
            ...
            firefox = pkgs.writeShellApplication {
              ...
              text = ''
                firefox --kiosk '${cfg.url}'
              '';
            };
            script = if cfg.browser == "chromium" then chromium else firefox;
          in
