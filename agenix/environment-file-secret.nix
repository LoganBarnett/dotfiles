...
{ lib, ... }:
{
  ...
  age.generators.environment-variable =
    ...
    ''
      echo "${lib.escapeShellArg secret.settings.field}=$(${decrypt} ${lib.escapeShellArg (builtins.elemAt deps 0).file})"
    '';
  ...
  age.generators.environment-file =
    ...
    lib.strings.concatStringsSep "; echo ; " (
      builtins.map (dep: "${decrypt} ${lib.escapeShellArg dep.file}") deps
    );
}
