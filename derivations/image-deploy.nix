{ pkgs, disk-detachable, image-create }:
pkgs.writeShellScriptBin "image-deploy" ''
  ##
  ...
  if [ -z ''${disk+x} ]; then
    disk="/dev/$(${disk-detachable}/bin/disk-detachable | head -n 1)"
  fi
  ...
  if [ -z ''${image_name+x} ]; then
    if [[ $noop == false ]]; then
      if ! image_result="$(${image-create}/bin/image-create --host "$host")"; then
  ...
  if [[ "$image_name" =~ \.zst$ ]]; then
    ...
    if [[ $noop == false ]]; then
      ...
      if ! ${pkgs.zstd}/bin/unzstd --decompress $image_name \
